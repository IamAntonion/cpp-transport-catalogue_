...
public:
    ...
    std::optional<RouteInfo> BuildRoute(std::pair <VertexId, VertexId> id) const;
...
template <typename Weight>
std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(std::pair<VertexId,
                                                                              VertexId> id) const {
    const auto& route_internal_data = routes_internal_data_.at(id.first).at(id.second);
    ...
    for (std::optional<EdgeId> edge_id = route_internal_data->prev_edge;
         edge_id;
         edge_id = routes_internal_data_[id.first][graph_.GetEdge(*edge_id).from]->prev_edge) {
